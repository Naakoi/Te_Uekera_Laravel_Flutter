        ...
        >
            ...
            <div className="pb-24">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="mb-10 md:mb-16 pb-6 md:pb-8 border-b border-black/5 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="text-center md:text-left">
                            ...
                            <h2 className="mt-4 text-3xl md:text-5xl font-black text-[#1a1a1a] uppercase tracking-tighter italic font-sans">Available Editions</h2>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 px-2 md:px-0">
                        {documents.map((doc) => (
                            <div key={doc.id} className="group bg-white/70 backdrop-blur-2xl border border-white/40 shadow-xl hover:shadow-[0_32px_64px_-15px_rgba(0,0,0,0.2)] rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden transition-all hover:-translate-y-2 flex flex-col h-full">
                                <Link href={route((auth.user.role === 'staff' || auth.user.role === 'admin') ? 'documents.reader' : 'documents.show', doc.id)} className="block aspect-[3/4] overflow-hidden bg-[#f4f1ea] relative">
                                    ...
                                    <div className="absolute top-4 left-4 md:top-6 md:left-6 bg-[#be1e2d] text-white px-4 md:px-5 py-1.5 md:py-2 text-[10px] md:text-xs font-black uppercase tracking-tighter rounded-full shadow-lg shadow-red-900/40">
                                        ${doc.price}
                                    </div>
                                </Link>
                                <div className="p-6 md:p-10 flex-1 flex flex-col bg-white/30 backdrop-blur-sm border-t border-white/40">
                                    <h3 className="text-xl md:text-3xl font-black text-[#1a1a1a] uppercase tracking-tighter leading-none italic mb-4 md:mb-6 font-sans group-hover:text-[#be1e2d] transition-colors">{doc.title}</h3>
